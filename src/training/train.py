...
def run_meta_learning(conf: TrainingConfiguration, x: np.ndarray, y: np.ndarray):
    ...
    def meta_learner_factory(learner: Model):
        return lstm_meta_learner(learner=learner, hidden_state_size=conf.hidden_state_size,
    ...
    dummy_meta_learner = meta_learner_factory(dummy_learner)
    n_params = get_trainable_params_count(dummy_meta_learner.train_model)

    logger.info("Using Meta-Learner with {} parameters".format(n_params))
    dummy_meta_learner.train_model.summary()
    ...
    meta_learning_task = MetaLearningTask(
        ...
        learner_weights_path=os.path.join(log_dir, "learner_weights.h5"),
        debug_mode=conf.debug_mode,
        logger=logger)
